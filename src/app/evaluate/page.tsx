...
import { TgvAdvisoryPanel } from "@/components/desk/TgvAdvisoryPanel";
    ...
    <main className="mx-auto max-w-[1800px] px-4 py-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[360px_minmax(0,1fr)] 3xl:grid-cols-[400px_minmax(0,1fr)]">
        <form onSubmit={submit} className="panel space-y-4 lg:sticky lg:top-4 lg:self-start">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-desk-muted">
              Type Make / Model / Caliber. Max Bid still needs OA solds (catalog pick or auto-resolve).
            </p>
          </div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <FieldHint
              label="Make"
              hint="Manufacturer / brand as on the gun."
              v={form.manufacturer}
              on={(e) => {
                const manufacturer = e.target.value;
                setForm((f) => ({ ...f, manufacturer, gbaModelId: "", gbaCaliberId: "" }));
              }}
            />
            <FieldHint
              label="Model"
              hint="Model line (e.g. P320, Axis 2 XP)."
              v={form.model}
              on={(e) => {
                const model = e.target.value;
                setForm((f) => ({ ...f, model, gbaModelId: "", gbaCaliberId: "" }));
              }}
            />
            <FieldHint
              label="Caliber"
              hint="Optional but tightens OA leaf when resolving."
              v={form.caliber}
              on={(e) => {
                const caliber = e.target.value;
                setForm((f) => ({ ...f, caliber, gbaModelId: "", gbaCaliberId: "" }));
              }}
            />
            <div>
          ...
          <details className="text-xs text-desk-muted">
            <summary className="cursor-pointer">OA catalog pick (for Max Bid leaf)</summary>
            <p className="mt-1 text-[11px]">
              Optional — fills Make/Model/Caliber and locks the OA model/caliber IDs used for sold comps.
            </p>
            <div className="mt-2">
              <OaCatalogPickers
                condition={isVendor ? "new" : isTradeIn ? "used" : form.condition}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {!isVendor && (
              <details className="col-span-2 text-xs text-desk-muted">
                <summary className="cursor-pointer">UPC / MPN (optional)</summary>
          ...
          <button
            type="submit"
            disabled={loading || !form.manufacturer.trim() || !form.model.trim()}
          ...
          {form.manufacturer.trim() && form.model.trim() && !form.gbaModelId && (
            <p className="text-center text-[11px] text-desk-muted">
              No OA leaf locked — will auto-resolve for Max Bid when possible. TGV shows if OA misses.
            </p>
          )}
        ...
        <section className="space-y-4">
          {!data && !loading && (
            <div className="panel space-y-3 py-10 text-center text-sm text-desk-muted">
              ...
              <ol className="mx-auto max-w-md space-y-1 text-left text-xs">
                <li>
                  1. Type <strong className="text-desk-text">Make / Model / Caliber</strong> (optional OA
                  catalog pick for Max Bid leaf)
                </li>
                <li>
                  2. Set min profit + Local tax and GunBroker fees (both exits always run)
                </li>
                <li>
                  3. Get <strong className="text-desk-text">Local + GunBroker max bids</strong> when OA
                  solds exist; TGV advisory if OA misses
                </li>
              </ol>
            </div>
          )}
          ...
          {data.tgvAdvisory && <TgvAdvisoryPanel tgv={data.tgvAdvisory} />}
